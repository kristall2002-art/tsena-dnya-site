#!/usr/bin/env python3
"""
SSG (Static Site Generation) для сайта "Цена Дня WB".
...
BASE_URL = "https://kristall2002-art.github.io/tsena-dnya-site"
...
# ─── WB image URL (same algorithm as JS) ───────────────────────────
BASKETS = [
    ...
    (3299, 19), (3491, 20), (3699, 21), (3919, 22), (4099, 23), (4299, 24),
    (4499, 25), (4699, 26), (4999, 27), (5199, 28), (5599, 29), (5999, 30),
    (6199, 31), (6599, 32), (6799, 33), (7099, 34), (7399, 35), (7699, 36),
    (7999, 37),
]


def wb_image_url(article_id: int) -> str:
    vol = article_id // 100000
    part = article_id // 1000
    basket = 38 + (vol - 8000) // 300
    for limit, num in BASKETS:
        if vol <= limit:
            basket = num
            break
    basket = min(basket, 99)
    return f"https://basket-{basket:02d}.wbbasket.ru/vol{vol}/part{part}/{article_id}/images/c516x688/1.webp"


# ─── Price helpers (same as JS) ────────────────────────────────────
def wallet_to_site(wallet_price: int) -> int:
    approx = round(wallet_price / 0.98)
    for s in range(approx - 2, approx + 3):
        if s - math.ceil(s * 0.02) == wallet_price:
            return s
    return approx
...
:root{
  ...
  --max-w:1200px;
  --price-color:#fff;
  --header-bg:linear-gradient(135deg,var(--purple-dark),#4a2578);
  --shadow-color:rgba(107,63,160,.4);
}
html.light{
  --bg:#f5f0fa;
  --bg2:#fff;
  ...
  --text3:#8b7da0;
  --price-color:#1a1328;
  --shadow-color:rgba(107,63,160,.15);
}
body{
...
.header{
  background:linear-gradient(135deg,var(--purple-dark),#4a2578);
  padding:14px 20px;
  z-index:100;
  box-shadow:0 2px 20px rgba(107,63,160,.4);
}
...
.card:hover{
  ...
  box-shadow:0 8px 25px rgba(107,63,160,.3);
...
.product-wallet{
  display:flex;align-items:center;gap:6px;
  font-size:15px;color:var(--purple-light);margin-bottom:16px;
}
...
.btn-wb{
  ...
  padding:14px 28px;
  background:linear-gradient(135deg,#cb11ab,#7b2ff2);
  color:#fff;border:none;border-radius:12px;
  font-size:16px;font-weight:700;cursor:pointer;
  text-decoration:none;transition:opacity .2s;
}
.btn-wb:hover{opacity:.85}
.btn-track{
  display:inline-flex;align-items:center;justify-content:center;gap:8px;
  padding:12px 28px;
  background:var(--bg3);
  ...
  <div class="header-inner">
    ...
    <div class="header-center">
      <div class="header-row1">
        <a href="{base_path}" style="text-decoration:none;color:inherit"><h1 class="logo">Цена Дня <span>| Wildberries</span></h1></a>
        <div class="header-sub">Поиск самых низких цен на товары каждый день</div>
      </div>
      <div class="header-row2">
        <span class="header-tg">Ещё больше товаров в нашей Telegram-группе 👉 <a href="https://t.me/tsena_dnya_wb" target="_blank" style="color:#fff;text-decoration:underline">Цена Дня | WB</a></span>
      </div>
    </div>
...
def build_footer() -> str:
    return """\
<footer class="footer">
  Цена Дня | WB — лучшие цены на Wildberries каждый день<br>
  <a href="https://t.me/tsena_dnya_wb" target="_blank">Telegram-канал</a> ·
  <a href="https://t.me/tsena_dnya_bot" target="_blank">Telegram-бот</a> ·
  Данные обновляются каждые 10 минут<br>
  <a class="ad-btn" href="https://t.me/tsena_dnya_bot?start=ad" target="_blank">📢 Заказ рекламы на сайте и в Telegram-канале</a>
</footer>"""
...
def get_product_image_html(p: dict) -> str:
    if p.get("image"):
        ...
        img = wb_image_url(int(p["article"]))
        return f'<img class="card-img" src="{img}" alt="{esc(p.get("name", ""))}" loading="lazy" onerror="this.outerHTML=\'<div class=\\\'card-img no-img\\\'>📦</div>\'">'
...
def get_product_image_url(p: dict) -> str:
    ...
    if p.get("image"):
        ...
        return wb_image_url(int(p["article"]))
    return ""


def build_card_html(p: dict, cat_name: str = "", cat_emoji: str = "", base_path: str = "../../") -> str:
    site_price = wallet_to_site(p["price"])
    has_discount = p.get("oldPrice") and p["oldPrice"] > site_price
    discount = round((p["oldPrice"] - site_price) / p["oldPrice"] * 100) if has_discount else 0
...
<a class="card" href="{product_url}">
  ...
  <div class="card-body">
    ...
    <div class="card-prices">
      <span class="price-now">{format_price(site_price)}</span>
      {discount_html}
    </div>
    <div class="wallet-price"><span class="wallet-icon">💜</span> С WB Кошельком: {format_price(p["price"])}</div>
...
def generate_category_page(
    ...
    title = f'{cat_info["emoji"]} {cat_info["name"]} — Цена Дня | WB'
    desc = f'Лучшие цены на {cat_info["name"].lower()} на Wildberries. {len(products)} товаров со скидками. Обновлено каждый день.'
    base = "../../"
...
def generate_product_page(
    ...
    site_price = wallet_to_site(product["price"])
    has_discount = product.get("oldPrice") and product["oldPrice"] > site_price
    discount = round((product["oldPrice"] - site_price) / product["oldPrice"] * 100) if has_discount else 0
    wb_url = f"https://www.wildberries.ru/catalog/{article}/detail.aspx" if article else "#"
    track_url = f"https://t.me/tsena_dnya_bot?start=track_{article}" if article else "#"
    base = "../../"
    ...
    desc += ". Лучшая цена на Wildberries."
    ...
    if product.get("image"):
        ...
        img_src = wb_image_url(int(article))
        img_html = f'<img class="product-image" src="{img_src}" alt="{esc(name)}" onerror="this.outerHTML=\'<div class=\\\'product-image no-img\\\'>📦</div>\'">'
    else:
...
<main class="main">
  <div class="product-page">
    ...
    <div class="product-info">
      ...
      <div class="product-wallet"><span class="wallet-icon">💜</span> С WB Кошельком: {format_price(product["price"])}</div>
      <div class="product-rating">
        <span class="star">⭐</span> {esc(product.get("rating", "—"))} · {esc(product.get("reviews", ""))}
      </div>
      <div class="product-actions">
        <a class="btn-wb" href="{wb_url}" target="_blank" rel="noopener">🛒 Купить на Wildberries</a>
...
def main():
    ...
    # Also skip special keys like "digest", "weekly"
